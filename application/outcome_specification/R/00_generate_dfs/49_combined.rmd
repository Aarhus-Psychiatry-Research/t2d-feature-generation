...
df_first_diabetes_any <- df_first_t2d_blood_sample %>% 
  rename(df_first_t2d_blood_sample = datotid_start) %>% # Add BS
  left_join(df_first_date_of_t2d_medication_prescription) %>% 
  left_join(df_first_t2d_diagnoses_combined) %>% 
  mutate(datotid_first_diabetes_any = pmin(datotid_first_t2d_medication, datotid_first_t2d_diagnosis, df_first_t2d_blood_sample, na.rm = TRUE)) %>% 
...
df_first_t2d_bs_only <- df_first_t2d_blood_sample %>% 
  rename(df_first_t2d_blood_sample = datotid_start) %>% # Add BS
  left_join(df_first_date_of_t2d_medication_prescription) %>% 
  filter(df_first_t2d_blood_sample < datotid_first_t2d_medication) %>% 
  left_join(df_first_t2d_diagnoses_combined) %>% 
  filter(df_first_t2d_blood_sample < datotid_first_t2d_diagnosis) %>% 
  group_by(dw_ek_borger) %>% 
  filter(df_first_t2d_blood_sample == min(df_first_t2d_blood_sample)) %>% # Make sure to only have one record per patient
  select(dw_ek_borger, df_first_t2d_blood_sample) %>% 
  distinct(dw_ek_borger, df_first_t2d_blood_sample) %>% 
  left_join(df_first_psych_visit) %>% # 3010
  filter(datotid_first_psych_visit < df_first_t2d_blood_sample) %>% # Keep only if diabetes is diagnosed after first psych visit: 810
  left_join(rename(df_first_t1d_diagnoses_combined, datotid_first_t1d_diagnosis = datotid_start), by = "dw_ek_borger") %>% 
  mutate(!(datotid_first_t1d_diagnosis < df_first_t2d_blood_sample)) %>% 
  filter(is.na(datotid_first_t1d_diagnosis) | !(datotid_first_t1d_diagnosis < df_first_t2d_blood_sample)) %>% # Keep only if no t1d diagnosis before t2d: 601
  select(dw_ek_borger, df_first_t2d_blood_sample) %>% 
  rename(timestamp = df_first_t2d_blood_sample)
